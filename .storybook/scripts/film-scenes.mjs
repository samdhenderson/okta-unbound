...
import { CHOREOGRAPHY, REEL_ORDER, SCENE_TITLES } from './scenes/choreography.mjs';
 ...
 * `window`, `addEventListener` and `setTimeout` — so serializing it with
 * `toString()` keeps `showcase.mjs` the single source of truth rather than
 * forcing a second, drifting copy inline here.
 */
const STAGE_INIT = `
...
/**
 * Load a scene and wait until it is genuinely painted.
 *
 ...
 */
async function loadScene(page, id) {
...
// discovers and pre-bundles the app's dependency graph.
{
  ...
  await loadScene(page, scenes[0].id).catch(() => {});
  await warm.close();
...
if (REEL) {
  ...
  for (const [i, scene] of ordered.entries()) {
    ...
    await loadScene(page, scene.id);

    // The card goes up over the freshly-staged panel and covers the last of its
    // paint, which is also what hides the tab-load spinner between scenes.
    await page.evaluate(
      (spec) => window.__DEMO_STAGE__?.titleCard(spec),
      { index: i + 1, total: ordered.length, ...SCENE_TITLES[scene.id] },
    );
    await new Promise((r) => setTimeout(r, 3400));
    await page.evaluate(() => window.__DEMO_STAGE__?.dropTitleCard());
    await new Promise((r) => setTimeout(r, 900));
    await page.mouse.move(STAGE.width - PANEL.width - 260, STAGE.height / 2);

    const beats = [];
    const beat = async (name, movement) => {
      const bAt = Date.now() - reelStarted;
      try {
        await movement();
        beats.push({ name, at: bAt, ok: true });
      } catch (err) {
        beats.push({ name, at: bAt, ok: false, error: String(err?.message ?? err) });
        console.warn(`\n    ! ${scene.name}/${name}: ${err?.message ?? err}`);
      }
    };
    await CHOREOGRAPHY[scene.id](page, beat);
    await new Promise((r) => setTimeout(r, 900));
  ...
  // End card.
  await page.evaluate(() =>
    window.__DEMO_STAGE__?.titleCard({
      title: 'Okta Unbound',
  ...
  const failed = chapters.flatMap((c) => c.beats.filter((b) => !b.ok).map((b) => `${c.id}/${b.name}`));
...
for (const scene of scenes) {
  ...
  // through the scene's opening caption.
  await page.mouse.move(STAGE.width - PANEL.width - 260, STAGE.height / 2);

  // The panel's own entrance plays here; everything after this is content.
  ...
  const beat = async (name, movement) => {
    ...
    try {
      await movement();
      beats.push({ name, at, ok: true });
    } catch (err) {
      beats.push({ name, at, ok: false, error: String(err?.message ?? err) });
      console.warn(`\n  ! ${scene.name}/${name}: ${err?.message ?? err}`);
    }
  };
...
const missed = manifest.flatMap((m) => m.beats.filter((b) => !b.ok).map((b) => `${m.id}/${b.name}`));
