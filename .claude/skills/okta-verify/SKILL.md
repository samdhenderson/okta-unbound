...
| 10  | `npm run lint:cited-paths`                                           | Every `src/…` path cited as a backticked span or a markdown link target still resolves on disk. Scope: tracked `.md` under `docs/` and `.claude/`, the root pointers `CLAUDE.md` / `AGENTS.md`, and the ledgers `DEBT.md`, `IMPROVEMENTS.md`, `SESSION.md`, `CONVENTIONS.md`. `NIGHTLY.md` is excluded — it is an append-only log of the repo as it was. Globs, directory references, and fenced code blocks are not treated as citations | **Hard** — `verify` job                                                                                                                                                                                                                                                         |
...
— every story runs as a headless-browser render test and must be axe-clean, and an
a11y violation fails CI (`docs/testing.md`). It does not check that the right text
...
story" is never on its own grounds to delete a `.test.tsx` under the duplicate-coverage
carve-out (`docs/testing.md`) — a
