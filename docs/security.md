...
[How to verify](#how-to-verify-independently).
...
| Data at rest     | No credentials in storage; TTL'd caches, capped undo history (now holds prior profile values — §8 #10), capped/redacted request log, user-configurable audit retention |
...
- **Boundary validation (zod).** [`shared/schemas/okta.ts`](../src/shared/schemas/okta.ts)
  ...
  the residual is unread payload held in memory, itemised at §8 #11.
...
- **Verbose API request log (bounded and redacted).**
  ...
  at §8 #12. No request or response body is recorded; an entry holds a reason, method,
