...
| `research:awaiting-review` item (architecturally significant, scoped as a decision record)        | general-purpose agent, **read-only, proposal-only** | Output is a Proposed-status decision record under `docs/adr/`, numbered next in sequence (the corpus restarted at `0001`; never reserve a number in advance). The PR touches `docs/` only — zero files under `src/`. Sam moves the item to `open` by accepting it. |
...
| A house rule or decision record now stale because of what shipped                                 | `docs-maintainer`                                   | Runs last, only if triggered                                                                                                                                                                                                                                       |
