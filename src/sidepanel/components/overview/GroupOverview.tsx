...
interface GroupOverviewProps {
  ...
  groupName: string;
  targetTabId: number;
  onTabChange: (tab: 'rules' | 'security' | 'users') => void;
}
...
const GroupOverview: React.FC<GroupOverviewProps> = ({
  ...
  const handleCustomCleanup = async () => {
    ...
    showProgress('Custom Cleanup', `Removing users with status: ${selectedStatuses.join(', ')}...`);
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          color="neutral"
          icon="bolt"
          subtitle={`${metrics.membershipSources?.direct ?? 0} manual`}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          {/* Total Members */}
          <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Total Members</h3>
            <div className="flex items-center justify-center">
              <div className="text-center">
                <div className="text-5xl font-bold bg-gradient-to-r from-[#007dc1] to-[#3d9dd9] bg-clip-text text-transparent">
                  {(metrics.totalMembers ?? 0).toLocaleString()}
                </div>
                <div className="text-sm text-gray-600 mt-2">members in this group</div>
              </div>
            </div>
          </div>
        </div>
