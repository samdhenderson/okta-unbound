              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {group.isPushGroup && group.linkedGroups && group.linkedGroups.length > 0 && (
                    <span
                      className="px-2.5 py-1 rounded-md text-xs font-bold bg-cyan-50 text-cyan-700 border border-cyan-200 inline-flex items-center gap-1"
                      title={`Pushed to: ${group.linkedGroups.map(lg => lg.sourceAppName || 'App').join(', ')}`}
                    >
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
                      </svg>
                      {group.linkedGroups.length === 1
