...
export function useTabState(tabName: string) {
  ...
  // Save search query
  const setSearchQuery = React.useCallback(async (query: string) => {
    await manager.updateProperty('searchQuery', query);
    setState(prev => ({ ...prev, searchQuery: query }));
  }, [manager]);

  // Save filters
  const setFilters = React.useCallback(async (filters: Record<string, any>) => {
    await manager.updateProperty('filters', filters);
    setState(prev => ({ ...prev, filters }));
  }, [manager]);

  // Save scroll position
  const setScrollPosition = React.useCallback(async (position: number) => {
    await manager.updateProperty('scrollPosition', position);
    setState(prev => ({ ...prev, scrollPosition: position }));
  }, [manager]);

  // Update any property
  const updateState = React.useCallback(async (updates: Partial<TabState>) => {
...
// Note: React import above is just for the hook signature.
